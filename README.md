...
## Раздел 7: Настройка Вебхука для Локальной Разработки

Для того чтобы Telegram мог отправлять обновления вашему локально запущенному боту, необходимо настроить вебхук с использованием публичного URL, предоставляемого `localtunnel` (или аналогичным сервисом). Скрипт `set_telegram_webhook.sh` в корне проекта автоматизирует этот процесс.

**Перед использованием скрипта:**

1.  **Установите `localtunnel`:** Если еще не установлен, выполните `npm install -g localtunnel`.
2.  **Настройте `TELEGRAM_BOT_TOKEN`:**
    *   **Вариант А (Рекомендуемый):** Установите переменную окружения `TELEGRAM_BOT_TOKEN` в вашем терминале перед запуском скрипта:
        ```bash
        export TELEGRAM_BOT_TOKEN="ВАШ_ТЕЛЕГРАМ_БОТ_ТОКЕН"
        ```
    *   **Вариант Б:** Отредактируйте файл `set_telegram_webhook.sh` и напрямую впишите ваш токен вместо `YOUR_TELEGRAM_BOT_TOKEN_HERE`. (Менее безопасно, не рекомендуется для версионирования).
3.  **(Опционально) Настройте порт и путь вебхука:**
    *   Скрипт по умолчанию использует порт `5125` (переменная `HLEB_LOCAL_PORT` в скрипте, которая может быть переопределена переменной окружения `HLEB_LOCAL_PORT`; если она не установлена, скрипт использует `5125`). Также используется путь `/telegram_bot_webhook` (переменная `TELEGRAM_WEBHOOK_PATH` в скрипте, также может быть переопределена переменной окружения).
    *   Вы можете изменить эти значения в скрипте или установить переменные окружения `HLEB_LOCAL_PORT` и `TELEGRAM_WEBHOOK_PATH` перед его запуском. Убедитесь, что `TELEGRAM_WEBHOOK_PATH` соответствует тому, что настроено в `hleb/config/telegram.php` (ключ `webhook_path`) и используется в `hleb/routes/map.php`.

**Запуск скрипта:**

1.  **Запустите Docker-контейнеры** вашего Hleb-приложения:
    ```bash
    docker-compose up -d
    ```
    Убедитесь, что приложение доступно локально (например, на `http://localhost:5125`).

2.  **Сделайте скрипт исполняемым** (если еще не сделали этого ранее):
    ```bash
    chmod +x set_telegram_webhook.sh
    ```

3.  **Запустите скрипт** из корневой директории проекта:
    ```bash
...
1.  Проверяет, установлен ли `localtunnel`.
2.  Проверяет, задан ли `TELEGRAM_BOT_TOKEN`.
3.  Запускает `localtunnel` в фоновом режиме для указанного порта.
4.  Ожидает получения публичного URL от `localtunnel` (до 15 секунд).
5.  Формирует полный URL для вебхука (например, `https://yoursubdomain.loca.lt/telegram_bot_webhook`).
6.  Отправляет запрос к Telegram API для установки этого URL в качестве вебхука для вашего бота.
7.  Выводит результат операции и PID процесса `localtunnel`.

**После успешного выполнения:**

*   Ваш бот готов принимать обновления от Telegram на локальном сервере.
*   `localtunnel` будет продолжать работать в фоновом режиме. Чтобы остановить его позже, используйте команду `kill <PID>`, где `<PID>` — это идентификатор процесса, выведенный скриптом.
*   При каждом перезапуске `localtunnel` (и, следовательно, при каждом перезапуске вашего компьютера или если соединение `localtunnel` прервется) вам нужно будет **повторно запустить скрипт `set_telegram_webhook.sh`**, так как `localtunnel` будет генерировать новый публичный URL.

**Устранение неполадок:**
*   Если скрипт не может получить URL от `localtunnel`, проверьте файл `lt_output.log` (создается в корне проекта при работе скрипта) на наличие ошибок от `localtunnel`.
*   Убедитесь, что ваш `BOT_TOKEN` корректен и у бота нет уже установленного вебхука с ошибками (можно удалить вебхук через API Telegram, отправив запрос к `setWebhook` с пустым параметром `url`).
