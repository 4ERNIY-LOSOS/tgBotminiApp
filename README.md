...
*   Чтобы увидеть, какой публичный URL был получен от `localtunnel` и успешно ли установлен вебхук, проверьте логи сервиса `webhook-init`:
    ...
    ```
    В логах вы должны увидеть сообщения от скрипта `entrypoint-webhook.sh`, включая полученный URL и ответ от Telegram API. Этот публичный URL (например, `https://yoursubdomain.loca.lt`) вы можете использовать для `MINI_APP_BASE_URL` (если вам нужно знать его заранее) или для прямого доступа к вашему приложению через туннель.
