...
# MINI_APP_BASE_URL="" # Этот URL будет динамически получен при запуске в Docker, или задайте его для локальной разработки без Docker.
...
```
**Примечание:** При запуске через `docker-compose`, публичный URL для `MINI_APP_BASE_URL` теперь автоматически считывается приложением из файла, создаваемого сервисом `webhook-init` (который использует `localtunnel`). Это означает, что вам обычно **не нужно** вручную устанавливать `MINI_APP_BASE_URL` в `.env` файле, если вы работаете в Docker-окружении – приложение само определит актуальный `localtunnel` URL.
Значение `MINI_APP_BASE_URL` в `.env` может служить:
1. Фолбэком, если автоматическое определение по какой-либо причине не сработало.
2. Для локальной разработки без Docker, когда вы запускаете `localtunnel` вручную и хотите указать URL напрямую.
Сервис `webhook-init` по-прежнему будет выводить полученный от `localtunnel` URL в своих логах (`docker-compose logs webhook-init`), что может быть полезно для проверки или прямого доступа. Система автоматической настройки вебхуков для Telegram также использует этот динамически полученный URL.
...
## Раздел 7: Автоматическая Настройка Вебхука при Запуске Docker

Настройка вебхука для локальной разработки теперь автоматизирована с помощью нового Docker-сервиса `webhook-init`, который определен в `docker-compose.yml`.

**Принцип работы:**

1.  При запуске `docker-compose up -d` также стартует сервис `webhook-init`.
2.  Этот сервис использует `Dockerfile` из директории `docker/webhook-init/`, который устанавливает `localtunnel`, `curl` и запускает скрипт `entrypoint-webhook.sh`.
3.  Скрипт `entrypoint-webhook.sh` внутри контейнера `webhook-init`:
    *   Получает необходимые настройки (токен бота, путь вебхука, целевой сервис для туннеля) из переменных окружения, заданных в `docker-compose.yml` (которые, в свою очередь, могут браться из вашего `.env` файла).
    *   Запускает `localtunnel`, направляя его на ваш веб-сервис (по умолчанию `nginx` на порту `80` внутри Docker-сети).
    *   Пытается получить публичный URL от `localtunnel` (с несколькими попытками в случае неудачи).
    *   Использует полученный URL для автоматической установки вебхука Telegram для вашего бота через Telegram Bot API.
    *   После успешной установки вебхука, контейнер `webhook-init` продолжает работать, поддерживая активным туннель `localtunnel`.

**Как использовать:**

1.  **Убедитесь, что настроены переменные окружения в `.env` файле:**
    *   `TELEGRAM_BOT_TOKEN` (обязательно)
    *   `TELEGRAM_WEBHOOK_PATH` (по умолчанию `/telegram_bot_webhook`, если не задан в `.env` или `docker-compose.yml`)
    *   `TARGET_SERVICE_HOST` (по умолчанию `nginx`, если не задан)
    *   `TARGET_SERVICE_PORT` (по умолчанию `80`, если не задан)
    *   Опционально: `LT_SUBDOMAIN`, `MAX_LT_RETRIES`, `LT_START_TIMEOUT` для тонкой настройки поведения `localtunnel` внутри скрипта.

2.  **Соберите образ для `webhook-init` (если еще не собран):**
    ```bash
    docker-compose build webhook-init
    # или просто docker-compose build, если это первый запуск с новым сервисом
    ```
...
**Проверка статуса и получение URL:**

*   Чтобы увидеть, какой публичный URL был получен от `localtunnel` и успешно ли установлен вебхук, проверьте логи сервиса `webhook-init`:
    ```bash
    docker-compose logs webhook-init
    ```
    В логах вы должны увидеть сообщения от скрипта `entrypoint-webhook.sh`, включая полученный URL и ответ от Telegram API. Этот публичный URL (например, `https://yoursubdomain.loca.lt`) теперь автоматически передается в PHP-приложение для формирования ссылки на Mini App при работе в Docker. Вы также можете использовать этот URL из логов для прямого доступа к вашему приложению через туннель или для проверки.

*   Если сервис `webhook-init` завершился с ошибкой, логи также помогут понять причину (например, неверный токен, проблемы с `localtunnel`, недоступность целевого сервиса `nginx`).

**Остановка:**
*   При выполнении `docker-compose down` сервис `webhook-init` будет остановлен вместе с остальными, и туннель `localtunnel` закроется.
*   При следующем `docker-compose up -d` процесс настройки вебхука повторится с новым URL от `localtunnel`.

Эта автоматизированная система значительно упрощает запуск и тестирование бота в локальном Docker-окружении, избавляя от необходимости вручную запускать скрипт `set_telegram_webhook.sh` (этот скрипт будет удален в следующем шаге).
